...
function getBetweenYears(startYear, endYear) {
    ...
    return __spreadArray([], years, true).reverse();
}
/**
 * @description: 函数睡眠
 */
 ...
 */
var findParentNodeArray = function (array, parentSubjectCode, period) {
    if (period === void 0) { period = "value"; }
    if (!isArray(array)) {
        throw "传入的为非数组，请重新传入";
    }
    var parentSubjectStock = []; // 存储父节点
    var going = true; // 是否已找到要查到的节点
    var findParentNode = function (array, code) {
